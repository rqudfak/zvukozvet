    ...
    <form action="{{ route('announcements.store') }}" method="POST" class="announcement-form">
        ...
        <div class="form-group">
            ...
            <select name="type" id="type" required>
                <option value="Книга" {{ old('type') === 'Видеоигра' ? '' : 'selected' }}>Книга</option>
        ...
        <div class="form-group">
            ...
            <input type="text" name="languages" id="languages" value="{{ old('languages') }}" placeholder="Например: Русский, Английский" required>
            @error('languages')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="fragment" id="fragment" rows="15" required>{{ old('fragment')}}</textarea>
            @error('fragment')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('announcements.index') }}" class="btn-cancel">Отмена</a>
        </div>
    </form>
</div>
